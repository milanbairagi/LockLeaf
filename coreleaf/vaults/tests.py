...
class VaultBlobListCreateViewTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.item1_data = {"title": "test item 1", "username": "user1", "password": "pass1", "iv": "0123456789abcdef", "url":"https://example1.com", "notes": "notes1"}
        self.item2_data = {"title": "test item 2", "username": "user2", "password": "pass2", "iv": "0123456789abcdef", "url":"https://example2.com", "notes": "notes2"}
    ...
    def test_create_authenticated_user_item(self):
        ...
        self.assertEqual(response.data["iv"], self.item1_data["iv"])
    ...
    def test_create_authenticated_user_item_requires_iv(self):
        """Test that authenticated user cannot create an item without iv"""
        token = self.generate_token_for_user(self.user)
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")

        invalid_data = {"title": "test item", "username": "user1", "password": "pass1", "url": "https://example.com", "notes": "notes"}
    ...
    def test_list_authenticated_user_items(self):
        ...
        other_item = Item.objects.create(user=self.other_user, title="other item", username="otheruser", password="otherpass", iv="0123456789abcdef", url="https://other.com", notes="other notes")
...
class VaultBlobUpdateDestroyViewTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.item = Item.objects.create(user=self.user, title="test item", username="testuser", password="testpass", iv="0123456789abcdef", url="https://example.com", notes="test notes")
    ...
    def test_update_authenticated_user_item_cannot_change_iv(self):
        """Test that authenticated user cannot change an item's iv after creation"""
        token = self.generate_token_for_user(self.user)
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")

        response = self.client.patch(self.url, {"iv": "fedcba9876543210"}, format="json")

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.item.refresh_from_db()
        self.assertEqual(self.item.iv, "0123456789abcdef")
